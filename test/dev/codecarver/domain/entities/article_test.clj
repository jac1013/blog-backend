...
(deftest article-test
  (testing "Should return a map with error if title is greater than 50"
    (is (= true (contains? (validate {:action #() :to_validate {:title (r/string "a" 51) :body (r/string "qwer" 100)}}) :validation_error))))
  (testing "Should return a map with error if title is lower than 10"
    (is (= true (contains? (validate {:action #() :to_validate {:title (r/string "a" 8) :body (r/string "qwer" 100)}}) :validation_error))))
  (testing "Should return an empty function if title is between 10 and 50"
    (is (= true (empty? (validate {:action #() :to_validate {:title (r/string "a" 25) :body (r/string "qwer" 100)}})))))
